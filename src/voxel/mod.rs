...
impl Map {
	...
	pub fn change(
		...
		let mut entities: HashMap<Entity, MapChunkData> =
			HashMap::new();
		...
		let (existing, new): (Vec<_>, Vec<_>) = self
			...
			.into_iter()
			.partition(|(pt, _)| {
				existing_chunks.iter().any(|(_, _, p)| pt == p)
			});

		existing_chunks
			.into_iter()
			.filter(|(_, _, pt)| {
				existing.iter().any(|(p, _)| pt == p)
			})
		...
		let octrees = new
			...
			.map(|(pt, _)| {
				Octree::new(
					Aabb::new(
						Point::new(
							pt.x * self.chunk_dimensions.x
								+ self.chunk_dimensions.x / 2,
							pt.y * self.chunk_dimensions.y
								+ self.chunk_dimensions.y / 2,
							pt.z * self.chunk_dimensions.z
								+ self.chunk_dimensions.z / 2,
						),
						self.chunk_dimensions,
					),
					octree::DEFAULT_MAX,
		...
		entities.extend(
			self.insert_mapchunks_with_octrees(
				&octrees, world, false,
			),
		);
		...
		for (entity, map_data) in &mut entities {
			...
			let map_query_range =
				map_data.octree.query_range(overlap_aabb);
			let input_query_range = octree.query_range(overlap_aabb);

			let set = input_query_range
				.into_iter()
				.collect::<HashSet<TileData>>();
			let map_set = map_query_range
				.into_iter()
				.collect::<HashSet<TileData>>();

			if set.symmetric_difference(&map_set).count() == 0 {
				tracing::debug!(
					"Set and map_set were symmetrically the same"
				);
				continue;
			}
	...
	pub fn range_sliced_to_chunks(
		...
		let x_min_chunk = (min.x as f32
			/ self.chunk_dimensions.x as f32)
			.floor() as i32;
		let y_min_chunk = (min.y as f32
			/ self.chunk_dimensions.y as f32)
			.floor() as i32;
		let z_min_chunk = (min.z as f32
			/ self.chunk_dimensions.z as f32)
			.floor() as i32;

		let x_max_chunk =
			(max.x as f32 / self.chunk_dimensions.x as f32).floor()
				as i32 + 1;
		let y_max_chunk =
			(max.y as f32 / self.chunk_dimensions.y as f32).floor()
				as i32 + 1;
		let z_max_chunk =
			(max.z as f32 / self.chunk_dimensions.z as f32).floor()
				as i32 + 1;

		let min_chunk =
			Point::new(x_min_chunk, y_min_chunk, z_min_chunk);

		let dimensions =
			Point::new(x_max_chunk, y_max_chunk, z_max_chunk)
				- min_chunk;
	...
	pub fn query_chunk_range<
		T: IntoIterator<Item = (Entity, MapChunkData, Point)> + Clone,
	>(
		&self,
		map_datas: T,
		range: Aabb,
		...
		self.chunks_in_range(map_datas, range).iter().for_each(
			|(_, map_data)| {
		...
		if changed {
			let (components, map_chunks): (Vec<_>, Vec<_>) = octrees
				...
				.map(|octree| {
					...
					let area = self.chunk_dimensions.x
						* self.chunk_dimensions.z;

					(
						(
							ManuallyChange {
								ranges: vec![ChangeType::Direct(
									octree.get_aabb(),
								)],
							},
							chunk_pt,
