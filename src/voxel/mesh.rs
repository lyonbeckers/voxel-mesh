...
pub fn create_default_material_components_system(material: &'static str) -> impl systems::Runnable {
            ...
            for entity in entities {
                commands.exec_mut(move |world, _| {
                    if let Some(mut entry) = world.entry(entity) {
                        entry.add_component(custom_mesh::Material::from_str(material));
                    }
                })
            }
        })
...
pub fn create_visibility_notifier_system(node: Ref<Node>) -> impl systems::Runnable {
    SystemBuilder::new("visibility_notifier_system")
        .with_query(
            <(Entity, Read<MapChunkData>)>::query().filter(
                component::<custom_mesh::MeshData>() & !component::<VisibilityNotifierNode>(),
            ),
        )
        .build(move |commands, world, _, query| {
            let entities = query
            ...
            entities.into_iter().for_each(|(entity, map_data)| {
                commands.exec_mut(move |world, _| {
                    ...
                    let node =
                        unsafe { node::add_node(&node.assume_safe(), visibility_node.upcast()) };
                    let visibility_node = unsafe { node.assume_safe() }
...
pub fn create_drawing_system() -> Box<dyn FnMut(&mut World, &mut Resources)> {
    ...
    let mut changed_query = <(Entity, Read<VisibilityNotifierNode>)>::query().filter(
        !component::<Batched>() & component::<MapChunkData>() & component::<ManuallyChange>(),
    );
    ...
    Box::new(move |world, resources| {
        ...
        if let Some((entity, map_data, change, batch)) =
            batched_iter
                .next()
                .map(|(entity, map_data, change, batch)| {
                    (*entity, (*map_data).clone(), (*change).clone(), *batch)
                })
        {
            entities.push((entity, map_data, change));
            entities.extend(batched_iter.filter(|(_, _, _, b)| **b == batch).map(
                |(entity, map_data, change, _)| (*entity, (*map_data).clone(), (*change).clone()),
            ));
        }

        let (map_mesh_tx, map_mesh_rx) =
            crossbeam_channel::unbounded::<(Entity, HashMap<usize, VertexData>)>();
        ...
        if let Some(map) = resources.get::<Map>() {
            let chunk_volume =
                map.chunk_dimensions.x * map.chunk_dimensions.y * map.chunk_dimensions.z;

            loop {
                let combined_volume: i32 = entities
                    .par_iter_mut()
                    .map(|(_, map_data, _)| map_data.octree.count() as i32)
                    .sum();
            ...
            write_mesh_query.for_each_mut(world, |(entity, map_mesh_data, mesh_data, _)| {
                if let Some(map) = map_vert_datas.get_mut(entity) {
                    ...
                    map_mesh_data.cols.iter().for_each(move |vertex_data| {
                        ...
                        mesh_data
                            .indices
                            .par_extend(vertex_data.indices.par_iter().map(|i| i + offset));
        ...
        let (to_change_tx, to_change_rx) =
            crossbeam_channel::unbounded::<(Entity, Entity, Aabb, ChangeType)>();
                        ...
                        if let ChangeType::Direct(aabb) = change {
                            ...
                            neighbors
                                ...
                                .filter(|(_, neighbor_data)| {
                                    neighbor_data.get_chunk_point() != chunk_pt
                                })
                                .for_each(|(neighbor_entity, neighbor_data)| {
        ...
        to_change_rx
            .into_iter()
            .for_each(|(neighbor_entity, entity, map_aabb, change)| {
                let batched: Option<Batched> = world
                    .entry(entity)
                    .and_then(|entry| entry.get_component::<Batched>().ok().copied());
                ...
                if let Some(mut neighbor_entry) = world.entry(neighbor_entity) {
                    ...
                    if let ChangeType::Indirect(change_aabb) = change {
                        match neighbor_entry.get_component_mut::<ManuallyChange>() {
                            ...
                            _ => neighbor_entry.add_component(ManuallyChange {
                                ranges: vec![change],
                            }),
                        }
                    }
                    ...
                    manually_change.ranges = manually_change
                        ...
                        .filter(|c| match c {
                            ChangeType::Indirect(_)
                                if done_changes.iter().any(|(_, d)| d == *c) =>
                            {
                                false
                            }
                            _ => true,
        ...
        if map_data.octree.query_point(neighbor).is_none() {
            match map_data.octree.get_aabb().contains_point(neighbor) {
                false => {
                    ...
                    match map_datas
                        .par_iter()
                        .find_first(|(_, _, pt)| *pt == chunk_point_dir)
                    {
                        Some((_, map_data, _)) => match map_data.octree.query_point(neighbor) {
                            Some(_) => return {},
                            None => {
                                tx.send(*dir).unwrap();
                            }
...
fn is_a_subdivision(point_y: f32) -> bool {
    (point_y >= START_REPEAT_ABOVE_HEIGHT
        && (point_y % REPEAT_AMOUNT_ABOVE - START_REPEAT_ABOVE_HEIGHT) % REPEAT_AMOUNT_ABOVE == 0.)
...
fn adjust_scaled_pts(
    ...
    if !open_sides.contains(&dir)
        && (open_sides.contains(&right_dir) || open_sides.contains(&right_diag))
    {
        let middle = if right_dir.x.abs() > right_dir.z.abs() {
            Vector3::new(right_dir.x as f32, right_dir.y as f32, right_dir.z as f32)
                * TILE_DIMENSIONS.x
                / 2.
        } else {
            Vector3::new(right_dir.x as f32, right_dir.y as f32, right_dir.z as f32)
...
fn define_verts_from_sides(
    ...
    let right_dir = nalgebra::Rotation3::<f32>::from_axis_angle(
        &Vector3D::y_axis(),
    ...
    if sides.contains(&dir) {
        ...
        match corner {
            Some(corner) => points.extend(&[scaled_right, scaled_left, corner]),
...
fn draw_walls(
    ...
    points.par_iter().enumerate().for_each_with(
        (new_pts_tx, new_norm_tx, new_uv_tx, new_uv2_tx, new_idx_tx),
        |(new_pts_tx, new_norm_tx, new_uv_tx, new_uv2_tx, new_idx_tx), (i, border_point)| {
            ...
            if i % 2 == 0 {
                ...
                if dir.z.abs() > 0 {
                    u = world_point.x * TILE_SIZE
                        + (border_point.x - world_point.x).abs() * TILE_SIZE;

                    next_u = world_point.x * TILE_SIZE
                        + (next_point.x - world_point.x).abs() * TILE_SIZE;
                    ...
                    u = world_point.z * TILE_SIZE
                        + (border_point.z - world_point.z).abs() * TILE_SIZE;

                    next_u = world_point.z * TILE_SIZE
                        + (next_point.z - world_point.z).abs() * TILE_SIZE;
                ...
                let mut vert_offset = if bottom < START_REPEAT_BELOW_HEIGHT {
                    ...
                    ((((bottom - START_REPEAT_ABOVE_HEIGHT) / REPEAT_AMOUNT_ABOVE).floor()
                        * REPEAT_AMOUNT_ABOVE)
                        - REPEAT_AMOUNT_BELOW)
                        * TILE_SIZE
                } else {
                ...
                if map_coords_to_world(point).y + std::f32::EPSILON > true_top - 1. {
                    let (mut u, mut next_u) = if dir.z.abs() > 0 {
                        ...
                        let mut next_u =
                            TILE_SIZE + (next_point.x - world_point.x).abs() * TILE_SIZE;
                        ...
                        let mut next_u =
                            TILE_SIZE + (next_point.z - world_point.z).abs() * TILE_SIZE;
                    ...
                    new_uv2_tx
                        .send((uv_idx + 2, Vector2::new(next_u, top_v)))
                        .ok();
                    new_uv2_tx
                        .send((uv_idx + 3, Vector2::new(next_u, bottom_v)))
                        .ok();
                } else {
                ...
                new_uv_tx
                    .send((
                        ...
                        Vector2::new(
                            u,
                            -1. - height * TILE_SIZE - bottom * TILE_SIZE + vert_offset,
                        ),
                    ))
                    .ok();
                //top of face
                new_uv_tx
                    .send((
                        uv_idx + 1,
                        Vector2::new(u, -1. - bottom * TILE_SIZE + vert_offset),
                    ))
                    .ok();
                ...
                new_uv_tx
                    .send((
                        uv_idx + 3,
                        Vector2::new(next_u, -1. - bottom * TILE_SIZE + vert_offset),
                    ))
                    .ok();
