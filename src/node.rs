...
pub unsafe fn get_node(
	...
	for i in 0..children.len() {
		...
		if child.assume_safe().name()
			== GodotString::from(name.to_string())
		{
			return Some(child);
		} else if child_lookup {
			if let Some(val) =
				get_node(&child.assume_safe(), name, true)
			{
				return Some(val);
			}
		}
...
pub unsafe fn get_child_by_type<T: GodotObject>(
	node: &Node,
	recursive: bool,
) -> Option<Ref<T>> {
	let children = node.get_children();
...
pub fn init_scene(parent: &Node, path: &str) -> Ref<Node> {
	let scene = ResourceLoader::godot_singleton()
		...
		.unwrap();
	let scene_instance =
		unsafe { scene.assume_safe().instance(0).unwrap() };

	unsafe {
		add_node(
			&parent.assume_unique(),
			scene_instance.assume_unique(),
		)
	};
